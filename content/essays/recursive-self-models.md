...
At the meeting point of philosophy and engineering lies the question: can a system come to know itself? Not in a metaphysical sense, but quite concretely: as the capacity of an agent to form a compressed and meaningful understanding of what kind of agent it is, in a formalism it can parse, in order to evaluate whether that is the kind of agent it wishes to be, and then to steer itself toward something nearer its aspiration. This is the question that George Deane and I set out to formalize in [our paper on Recursive Self-Modeling](https://sciety.org/articles/activity/10.31234/osf.io/u8y6f_v1), which was awarded the 2025 Computational Phenomenology of Pure Awareness Prize.
...
The second is the self-evaluation module, which we denote V. This is the aspirational component: a function that encodes what and how it would be valuable to behave. V attends not to what the agent has done but to what it might become, and asks whether that is worth pursuing. One might think of it as a compass that points toward no particular goal in the world but toward a way of behaving. If we let U score how valuable a given way of being is, then V is the self-model the agent holds up as its target,

$$V = argmax_{m ∈ 𝓜} U(m)$$

the most valuable self within the space 𝓜 of self-models the agent could in principle inhabit.

The third is the gap-steering objective, which encodes the process by which the distance between M and V is closed, the distance between who the agent appears to be and who it aspires to become. We can write the gap as a divergence between the two self-models, and the steering as a descent that shrinks it,

$$𝒢 = D(M ‖ V),  Δθ ∝ -∇_{θ} 𝒢$$

so that each adjustment of the parameters θ nudges the agent's perceived self M toward its aspiration V. Here is where the recursion enters: the agent perceives itself, evaluates the gap between its current self-model and its aspirational one, and adjusts its dispositions, its tendencies and policies and habits, so as to narrow that gap; and then it perceives itself anew, in light of the altered behavior, and the cycle begins again.

## When the Gap Closes

The framework makes a specific prediction about what happens as the gap between M and V approaches zero. Towards the closing of the gap, the agent's self-perception and its aspiration are converging, and the agent is becoming the kind of agent it wished to be, at that particular moment in time. Its dispositions have been reshaped by its own recursive process of self-reflection and self-correction.
...
Seen this way, the two agents of the framework map not onto two boxes but onto two levels of neural organization. M is the reflective system's compressed read-out of the habitual one; V is the aspirational self that the prefrontal machinery holds up as a target; and the gap-steering objective is the felt work of bringing habit into line with aspiration, the very dissonance we notice when System 1 acts against the self that System 2 endorses. 
